  ...
  async update(req, res) {
    if(!req.file){
      return res.status(400).json({
        error: true,
        code: 129,
        message: "Error: Selecione imagem valida. Apenas PNG ou JPG"
      })
    }
    ...
    await User.findOne({ _id: req.userId }, '_id fileName')
      .then((user) => {
    ...
    await User.updateOne({ _id: req.userId }, dadosImagem).catch((err) => {
      return res.status(400).json({
        error: true,
        code: 129,
        message: 'Error: Imagem do perfil não editado com sucesso!',
      });
    })
